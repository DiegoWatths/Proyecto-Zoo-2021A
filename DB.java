...
public class DB {
	...
	public String Mostrar(String sentencia) {
		...
		try {
			...
			while(rs.next()) {
				...
				+ ", Habitat: " +rs.getString("Habitat")+ ", Comida:"+rs.getString("Comida")
	...
	public void Editar(String obj[]) {
		try {
			this.pstmt = conn.prepareStatement("UPDATE Animales SET Clase=?,Nombre=?,Habitat=?,Comida=?,Horario=? WHERE Id= "+obj[0]);
